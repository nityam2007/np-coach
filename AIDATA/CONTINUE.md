...
- **Lead persistence/email repair (2026-08-31):** Contact and Get-a-Quote now create items through Directus `ItemsService` behind the secret-protected extension, preserving CMS cache invalidation/activity without depending on lead collection permissions. Customer/staff messages use separate booking-grade, row-locked MariaDB-safe delivery leases, private maintenance discovery and Email Logs; the form pages render dynamically to prevent stale Server Action references across deployments.
