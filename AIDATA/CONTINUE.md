...
- **Transactional email (2026-08-06):** replaced deferred Microsoft 365 auth with the internal no-auth Postfix relay; added reusable NP Coaches HTML/plain-text templates with copy in Directus settings; OTP now fails visibly on delivery failure; persisted contact/quote submissions send staff + customer messages; verified Stripe payments and manual Directus Paid changes send idempotent booking confirmations and lost-property customer + staff messages tracked per record; an authenticated CMS hook triggers delivery and hourly maintenance retries it.
- **Lead persistence/email repair (2026-08-31):** Contact and Get-a-Quote now create items through a system-scoped Directus `ItemsService` behind the authenticated, secret-protected extension, preserving every validated field plus CMS cache invalidation/activity without depending on public lead permissions. Quote persistence deliberately uses the live collection's established `trip_from`, `trip_to`, `trip_date`, `message`, `passengers` contract and additively supplies `return_date`/`coach_size`; one shared mapper converts that record to email data. Customer/staff messages use separate booking-grade, row-locked MariaDB-safe delivery leases, private maintenance discovery and Email Logs; the form pages render dynamically to prevent stale Server Action references across deployments.
...
- **Analytics, apex SEO and Directus Studio repair (2026-08-22):** GA4 `G-3GXSRDBP55` is blocked until explicit PECR consent, uses path-only page locations and keeps all advertising consent disabled. The old apex Search Console token is retained, policy/banner copy is CMS-driven, legal pages receive a one-time revisioned update, and LocalBusiness schema uses verified business facts plus live services. Directus 12.1's reproducible `init_runtime_dom_esm_bundler` Studio crash is resolved by pinned 12.3.0; an isolated Chrome run reached `Sign In · Directus`, and the local Compose stack migrated, loaded the NP extensions present at that release and returned `pong`; production now also requires the payment-events hook.
- **CMS media and booking UX repair (2026-08-22):** all 18 single-file image/video fields now receive idempotent `directus_files` relations (the missing relations prevented File Library selection). The hero video retries explicit muted playback and falls back to its poster on a media error; the services motion control is labelled as a carousel. Booking prices no longer disappear when inventory is closed, and `service_runs` remains visible with clear automatic-inventory guidance. A follow-up aligns the seat display with the atomic endpoint: missing inventory infrastructure shows unavailable instead of a misleading 60/60 or sold-out result; production extensions are baked into the Directus image.
...
7. All Directus usages are pinned to 12.3.0; pin the standalone stack's remaining third-party images before that alternative is ever used.
...
**Forms (P3):** server actions in `src/app/actions.ts` (`submitContact`, `submitQuote`, `startBooking`, `startPassPurchase`); validation/helpers in `src/lib/forms.ts`; contact/quote writes use the private Directus extension and its `ItemsService` after zod, honeypot, rate-limit and Turnstile checks. Submission collections have **no anonymous create/read permission**.
...
- **Leads:** `contact_submissions`, `quote_requests` (private extension → Directus `ItemsService`; no anonymous access)
...
A production WordPress/Directus/code audit found that the current one-row-per-direction `routes` model does not represent the four real daily London–Midlands buses. There are two 60-seat services in each direction; Leicester is Friday–Monday and sold by the driver only. The previous booking resolver could take the first overlapping route and display a Leicester fare for a London–Midlands stop pair; the scheduled-service resolver removes that path.

The additive application implementation includes the scheduled-services model, explicit outward/return selection, stable-code validation, service/date inventory, immutable booking snapshots, multi-service timetables, Leicester exclusions, complete ticket details, and dated booking cards with local times, duration, fare, stop addresses and private-inventory seat availability.
...
The Stripe return page also falls back to the already-paid webhook result by its high-entropy Checkout Session id. This fixes zero-total checkouts being shown as “confirming” when the signed webhook had committed the ticket but the return page's second Stripe/API/email pass failed transiently; the fallback can only reveal a paid record and cannot promote pending inventory.

Authenticated ticket detail uses the same normalized-email, paid and committed Directus filters as the account list, with the booking reference added to that server-side query. This prevents a paid ticket visible in My Account from incorrectly reaching the generic 404 while retaining non-disclosing cross-customer authorization.

Both the paid webhook fallback and authenticated ticket page retry an unsent booking confirmation through the existing atomic delivery lease. A delivery failure never hides the ticket, and Stripe, the success page, ticket access, CMS Paid hooks and hourly maintenance still converge on one logical email rather than intentionally duplicating it.

The payment-email lease now has dedicated private Directus claim/finish transactions with row locks. This replaces the generic CAS of nullable delivery timestamps that left live paid booking 16 at `confirmation_email_status=pending`. Finish checks tolerate MariaDB removing timestamp milliseconds but still reject a stale worker after the ten-minute re-claim window. After Stripe verifies a session, success-page recovery prefers its server-authored metadata reference, avoiding scoped-token field-filter limitations on `stripe_session_id`.

Paid Daily Express bookings now have two independent delivery channels: customer ticket and staff operations. `BOOKING_TO` defaults to `bookings@np-coaches.co.uk`; both channels have their own status/start/sent fields and Email Logs keys, so one mailbox failure cannot suppress or falsely complete the other. Post-payment delivery uses the already-known Directus item id across Stripe, CMS reconciliation, ticket access and maintenance, removing the extra reference-filter read that could leave a paid record untouched.

Booking and lost-property success pages also have a final paid-only lookup by the exact high-entropy Checkout Session id. It cannot promote a pending record; Daily Express still requires committed inventory. This keeps a transient failure inside the primary verifier from showing “confirming” after Directus already has an authoritative paid order.

The Directus extension must send native JavaScript `Date` values to Knex for MariaDB timestamp fields. ISO strings containing `T`, milliseconds and `Z` left both delivery channels pending before SMTP or Email Logs were reached. Success pages now perform their paid/session-bound Directus lookup first and call Stripe only when the webhook has not yet committed the order.
