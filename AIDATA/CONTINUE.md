...
4. **Directus "custom permission rules" are edition-gated:** item filters and field allowlists return 403 (`custom_permission_rules_enabled is a restricted resource`) without an entitlement. Anonymous published-only reads are therefore enforced by `directus-extension-np-published` at the API boundary and repeated in every website CMS query; public content permissions use the standard Directus shape. Media bootstrap preserves an existing public-file permission during 12.1 upgrades instead of attempting a gated rewrite. Coolify bakes both extensions into `Dockerfile.directus`; do not recreate an `/directus/extensions` directory mount. Server actions still gate exactly what is written.
