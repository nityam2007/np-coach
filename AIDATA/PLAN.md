...
- **Payment/email observability ✅:** Stripe subtotal, promotion discount and final total are persisted for all valid discount percentages, including 100% sessions with no PaymentIntent. Paid state and ticket visibility are independent of SMTP success; stable delivery retries feed a metadata-only Directus Email Logs overview. Singular CMS pages expose every safe field read-only where appropriate, with authentication hashes still hidden. Canonical public origins come from `SITE_URL`/`NEXT_PUBLIC_SITE_URL` before the CMS fallback.
