...
- **P3 — Forms (storage + email ✅):** `/contact-us` + `/get-a-quote` use protected server actions with zod, honeypot, rate limiting and Turnstile. Valid leads persist through the secret-protected Directus extension before independent customer/staff Postfix channels run with MariaDB-safe leases, Email Logs and hourly retry. The public role has no create/read access; real mailbox receipt remains production acceptance work.
- **P4 — Daily Express booking ✅ (storage+payment+email done):** **stop-to-stop** like the live site — `/daily-express-service/book` → pick any From/To from a 6-stop corridor (`stops` collection) + single/return + date + passengers → server prices the leg from the flat corridor fares in `settings.pricing` (`dailyExpressSingle`/`Return`, pence + `dailyExpressVat` — **never trusts the client**) → `bookings` row (`from_stop`/`to_stop`) created `pending` → **Stripe Checkout (hosted)** → signed, **idempotent** webhook (`/api/stripe/webhook`, `checkout.session.completed`, unique `stripe_session_id`, routes by `metadata.kind`) atomically creates paid-only dated inventory and flips `pending → paid` → `/booking/success`. `bookings` is server-write only. Stripe-paid and CMS-paid bookings send independently tracked branded customer-ticket and staff-operation emails idempotently. Graceful "being set up" notice when `STRIPE_SECRET_KEY` is unset.
...
- **P5 — Lost Property pass ✅ (payment+email done):** `/lost-property/claim` form (mirrors the live-site fields: travel date/time, school-route school+route, where left, item description, two consents) → fee + VAT computed server-side from CMS `settings.pricing` (`lostPropertyFee` net + `lostPropertyVat` %, default £5 + 20% = £6.00) → `pass_purchases` row `pending` → **Stripe Checkout** → shared signed/idempotent webhook (`metadata.kind` routes to the right collection) → `paid` → `/pass/success`. **Configurable VAT** lives in `settings.pricing` with separate rates per fee (`lostPropertyVat`, `dailyExpressVat`) — re-seeding never clobbers client-edited rates. Stripe-paid and CMS-paid passes send branded customer and staff emails idempotently.
...
- **Payment/email observability ✅:** Stripe subtotal, promotion discount and final total are persisted for all valid discount percentages, including 100% sessions with no PaymentIntent. Paid state and ticket visibility are independent of SMTP success; native-Date MariaDB leases and stable delivery retries feed a metadata-only Directus Email Logs overview. Singular CMS pages expose every safe field read-only where appropriate, with authentication hashes still hidden. Canonical public origins come from `SITE_URL`/`NEXT_PUBLIC_SITE_URL` before the CMS fallback.
