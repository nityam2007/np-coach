...
- [x] **T6 — Forms (P3).** `contact_submissions` + `quote_requests` are written only by protected server actions through the secret-protected Directus extension; the public role has no create/read access. Server actions in [`src/app/actions.ts`](../src/app/actions.ts) perform zod validation, honeypot, rate limiting, and Cloudflare Turnstile before durable persistence. Customer and staff Postfix messages use independent MariaDB-safe delivery leases, Email Logs and hourly retry; real mailbox receipt remains production acceptance work.
...
- ✅ **P4** Daily Express booking (Stripe) — done. `/daily-express-service/book` → server-priced from Directus `routes` → `pending` `bookings` row → **Stripe Checkout** with customer-entered promotion codes → signed idempotent webhook `/api/stripe/webhook` records Stripe's verified discounted total, atomically commits paid-only dated inventory → `paid` → `/booking/success`. Fares editable in CMS (`price_single`/`price_return`, pence). Verified: server price, idempotency, unique session id, bookings locked to public, and branded ticket email for Stripe-paid or CMS-paid records.
...
- ✅ **P5** Lost Property pass (Stripe) — done. `/lost-property/claim` (live-site fields) → fee + VAT server-computed from CMS `settings.pricing` (£5 + 20% = £6.00) → `pass_purchases` `pending` → **Stripe Checkout** with customer-entered promotion codes → shared webhook (`metadata.kind`) records Stripe's verified discounted total → `paid` → `/pass/success`. **Per-fee configurable VAT** in `settings.pricing` (`lostPropertyVat`/`dailyExpressVat`); re-seed preserves client-edited rates. Verified: £6.00 math, idempotency, unique session id, collection locked to public, and branded customer + staff emails for Stripe-paid or CMS-paid records.
...
- ✅ **CMS detail, promotion-code and email observability correction (2026-08-22)** — every singular Directus page shows all safe fields with technical data read-only and authentication hashes hidden; Email Logs follows Customer Sessions with status/attempt/error/source metadata. Stripe subtotal/discount/final totals now support 100%, 95%, 90% and future valid promotion codes, including no-PaymentIntent checkouts. Paid inventory/tickets complete independently of SMTP and failed messages remain retryable. Public origins resolve from environment configuration.
- ✅ **Zero-total Stripe return-page reconciliation (2026-08-22)** — if the signed webhook has already committed a paid order but the immediate return page's second Stripe/API/email pass fails, the page resolves that paid record by its high-entropy Checkout Session id. The fallback is read-only, requires paid/committed inventory for tickets, and cannot convert a pending order.
- ✅ **Account ticket-detail ownership query (2026-08-22)** — the detail route now fetches by normalized signed-in email + reference + paid/committed state in one Directus query, matching the successful My Tickets list and eliminating the false 404 without revealing another customer's reference.
- ✅ **Paid ticket email immediate retry (2026-08-22)** — the Checkout paid-state fallback and an authenticated customer opening their own unsent ticket both retry the existing leased confirmation channel. Ticket access never depends on SMTP, while Stripe/webhook, ticket access, CMS hooks and maintenance remain idempotent.
- ✅ **Transactional email lease correction (2026-08-22)** — replaced generic nullable-timestamp CAS with dedicated row-locked Directus claim/finish endpoints, including MariaDB fractional-timestamp tolerance. Paid Stripe return recovery uses the verified metadata reference before the session-id fallback, so a paid record cannot remain falsely “confirming” because of scoped technical-field filters.
- ✅ **Booking customer/staff dispatch correction (2026-08-22)** — paid Stripe, 100%-coupon and CMS-paid bookings now send independently leased customer-ticket and operational staff messages. `BOOKING_TO` is environment-configurable, Directus exposes both channel states, Email Logs tracks each logical message, and all retry entry points use the known item id instead of a second reference lookup. Success pages have a final session-bound, paid-only recovery that cannot promote pending orders.
- ✅ **MariaDB email timestamp correction (2026-08-22)** — Directus lease and Email Logs timestamps are written as native Date values through Knex, preventing MariaDB from rejecting ISO `T…Z` strings before SMTP starts. Paid success pages resolve the committed session-bound record before a secondary Stripe API read.
- ✅ **Contact/quote delivery repair (2026-08-31)** — both lead forms now persist every validated field through system-scoped Directus `ItemsService` behind the authenticated private extension, then use separate customer/staff row-locked delivery channels and private maintenance discovery. Quote writes and emails share the production `trip_from`/`trip_to`/`trip_date`/`message` contract, with additive return-date and coach-size fields. This keeps new rows immediately visible and complete in the CMS while removing public field filtering, schema-name drift, the create-only app-role/read mismatch, MariaDB ISO timestamp CAS failure, and stale static Server Action page risk.
...
- [x] Use the four supplied WordPress bus admin schedules as authoritative; all previously documented timetable conflicts are resolved to those records.
- [x] Seed all 48 numeric fare rows across the four online buses; Adult/Child/Infant share each supplied price and blank “Ex: 10” placeholders remain unavailable.
...
- [ ] Complete the remaining return/output acceptance tests from the brief. The suite enforces all four 60-seat timetables, the 15/12/15/6 numeric fare matrices, blank-row exclusions, cutoff, Leicester exclusion and atomic inventory.
