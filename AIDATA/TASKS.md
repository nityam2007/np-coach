...
- ✅ **Contact/quote delivery repair (2026-08-31)** — both lead forms now persist through Directus `ItemsService` behind the private extension, then use separate customer/staff row-locked delivery channels and private maintenance discovery. This keeps new rows immediately visible in the CMS while removing the create-only app-role/read mismatch, MariaDB ISO timestamp CAS failure, and stale static Server Action page risk.
