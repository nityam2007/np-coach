...
- ✅ **Contact/quote delivery repair (2026-08-31)** — both lead forms now persist every validated field through system-scoped Directus `ItemsService` behind the authenticated private extension, then use separate customer/staff row-locked delivery channels and private maintenance discovery. This keeps new rows immediately visible in the CMS while removing public field filtering, the create-only app-role/read mismatch, MariaDB ISO timestamp CAS failure, and stale static Server Action page risk.
