...
- `contact_submissions`, `quote_requests`, and `email_logs` are written through secret-protected internal Directus endpoints, so the scoped app role does not need direct access to those collections. CMS staff may read the leads and delivery overview according to their Directus role.
