...
>
> **Payments and forms:** prices are computed server-side; pending orders become paid only after a verified Checkout session or signed webhook. Pending or abandoned Daily Express checkouts do not consume seats or create Service Runs; payment confirmation commits the booking and dated inventory atomically. Stripe promotion codes, including 100% no-payment Checkout sessions, persist the verified subtotal, discount and actual paid total. Stripe and legitimate CMS Paid changes trigger the same idempotent ticket/customer/staff email delivery, with hourly retry. Email failure never rolls back a confirmed payment or hides its ticket; it remains visible for retry. Tickets and claim completion are hidden while payment is pending. Contact and quote submissions pass server-side validation, honeypot, rate-limit and Turnstile checks, then use the scoped Directus server token—there is no anonymous create permission.
>
