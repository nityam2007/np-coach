...
>
> **Payments and forms:** prices are computed server-side; pending orders become paid only after a verified Checkout session or signed webhook. Pending or abandoned Daily Express checkouts do not consume seats or create Service Runs; payment confirmation commits the booking and dated inventory atomically. Stripe promotion codes, including 100% no-payment Checkout sessions, persist the verified subtotal, discount and actual paid total. Stripe and legitimate CMS Paid changes trigger independent, idempotent customer-ticket and booking-staff email delivery, with hourly retry. A private row-locked Directus transaction owns each pending → sending → sent/failed email lease; native Date values keep its timestamp writes MariaDB-safe. Email failure never rolls back a confirmed payment or hides its ticket; it remains visible for retry. Stripe return pages prefer an already-paid webhook result bound to the Checkout Session before making a secondary Stripe API request, and authenticated ticket detail scopes owner email + reference + paid inventory in one query. Tickets and claim completion are hidden while payment is pending. Contact and quote submissions pass server-side validation, honeypot, rate-limit and Turnstile checks, then persist through the secret-protected Directus extension before independently tracked customer/staff email delivery—there is no anonymous create permission.
>
...
>
> **Analytics (2026-09-15):** GA4 `G-3GXSRDBP55` starts in cookieless mode before a choice, permits analytics cookies only after Accept, and stops measurement on Reject (including saved rejections on later visits). Advertising consent and Google signals remain disabled. Commands use Google's Arguments-object queue format. Page locations/referrers omit query strings and ticket references. Cookie/privacy notices are CMS-managed and match this behaviour. Cookieless requests still send limited information to Google; this implementation is not a legal compliance assessment. Search Console verification remains unchanged. Directus is pinned to 12.3.0.
> **CMS media and booking controls (2026-08-22):** additive bootstrap now repairs the real `directus_files` relation behind every single-file image/video field, so editors can choose or replace media from the File Library. `service_runs` remains visible as the paid-only, automatically-created per-date/per-departure inventory ledger. Scheduled Services hold the normal capacity (60 initially; editable up to 500 for multiple/larger buses), while Service Runs can override capacity or cancel one date. Live availability and checkout now require the same authenticated atomic inventory endpoint, so an infrastructure fault cannot be shown as a false sold-out result.
> **CMS records and email overview (2026-08-22):** singular Directus item pages expose every ordinary record field; transactional and technical fields are read-only unless they are an explicit operator control. OTP and session hashes remain hidden. **Email Logs**, directly after Customer Sessions, records one metadata-only row per logical customer/staff message with queued/sending/sent/failed state, attempt count, safe error code and source record—never the message body.
...
**GA4 acceptance check:** after deployment, visit with blockers disabled, choose Accept analytics, navigate to another page, and inspect your GA4 Realtime report for the configured stream. Cookieless measurement is not equivalent to full visitor/session reporting; Google's modeling has eligibility thresholds. No GA4 account access, real-browser verification or synthetic analytics requests were used for this patch. See [Google consent mode](https://developers.google.com/tag-platform/security/concepts/consent-mode) and [behavioral modeling](https://support.google.com/analytics/answer/11161109).
...
- The internal Postfix relay sends branded HTML/plain-text OTP, booking-ticket, lost-property, contact, and quote mail. OTP requests fail visibly when SMTP fails; every logical message is tracked in Directus Email Logs, paid booking/pass delivery retries independently of the authoritative paid state, and form acknowledgements/notifications run only after the submission is stored.
- “Staff email” means the internal recipient mailbox only: contact/lost-property use `info@np-coaches.co.uk`, while bookings/quotes use `bookings@np-coaches.co.uk`. `BOOKING_TO`, `CONTACT_TO`, `QUOTE_TO`, and `LOST_PROPERTY_TO` can override them without code changes. These recipients are not Directus users and consume no Directus Studio seats.
