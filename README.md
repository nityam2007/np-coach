...
>
> **Payments and forms:** prices are computed server-side; pending orders become paid only after a verified Checkout session or signed webhook. Pending or abandoned Daily Express checkouts do not consume seats or create Service Runs; payment confirmation commits the booking and dated inventory atomically. Payment emails use per-record delivery state so webhook retries and success-page verification do not intentionally send duplicates. Tickets and claim completion are hidden while payment is pending. Contact and quote submissions pass server-side validation, honeypot, rate-limit and Turnstile checks, then use the scoped Directus server token—there is no anonymous create permission.
>
