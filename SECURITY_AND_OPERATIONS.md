...
- Daily Express validates route direction, service day, date range, return journey, departure, cutoff, terms, CMS fare, and CMS capacity. After Stripe confirms payment, both journey legs and the paid booking transition are committed under row locks in one MariaDB transaction through a private authenticated Directus endpoint; the Next.js app never connects to MariaDB.
- Pending or abandoned Checkout rows do not create Service Runs or count against availability. Stripe payment confirmation atomically creates/updates dated inventory, commits both legs, stores run ids, and marks the booking paid; a secret-protected Directus hook reconciles the same inventory and email state after a legitimate manual CMS Paid change; replayed webhooks cannot double-deduct. Legacy holds from earlier releases remain safely releasable.
- Stripe references, Checkout sessions, and PaymentIntent relationships are unique. Paid, failed, expired, refunded, and disputed transitions are fail-closed and database-atomic. Stripe-verified subtotal, discount and actual total support partial and 100% promotion codes; no-payment sessions may legitimately have no PaymentIntent. Webhook and success-page retries share lease-token notification delivery state.
- Contact and quote records are persisted before email. A bounded maintenance retry worker uses atomic delivery leases; CMS-paid booking/pass reconciliation and payment/lead delivery failures are retried and remain visible in Directus. Payment finalisation is authoritative and independent of SMTP, so an email outage cannot hide a paid ticket or make Stripe retry a successfully processed webhook.
- The private authenticated Directus extension maintains metadata-only Email Logs (recipient, subject, source, status, attempts and safe error code; no body). Directus item pages expose complete safe records while keeping authentication hashes hidden and technical fields read-only.
- Paid-message delivery leases are claimed and completed under Directus row locks rather than generic nullable-field comparison. Lease completion tolerates sub-second timestamp normalization by MariaDB while rejecting a genuinely stale worker after re-claim.
...
2. Confirm the scoped `DIRECTUS_SERVER_TOKEN` can access only the required protected collections and fields. Confirm an unauthenticated `POST /np-internal/inventory/status` returns 401 rather than 404, incorrect secrets are rejected, the web availability API reports `inventoryReady: true`, Directus starts with `EXTENSIONS_MUST_LOAD=true`, and the payment-events hook is loaded.
3. Run the additive CMS bootstrap, verify `service_runs`, `email_logs`, amount subtotal/discount and new session/delivery/inventory fields exist, enter approved non-zero route capacities, and concurrency-test the last-seat case for single and return journeys before changing the booking flag.
...
7. Complete internal Postfix relay → Microsoft 365 mailbox delivery tests for contact, quote, OTP, booking, and lost-property customer/staff messages, including a forced failure followed by maintenance retry; reconcile each result in Email Logs.
