...
const COLLECTION_META = {
  ...
  scheduled_services: { group: "grp_daily_express", sort: 2, icon: "departure_board", display_template: "{{code}} · {{label}}", note: "Actual repeatable departures. Missing fare rows are not sold online; use Driver only for Leicester." },
  stops: { group: "grp_daily_express", sort: 3, icon: "pin_drop", display_template: "{{name}}", note: "The pickup/drop-off stops customers pick when booking. Don't change a stop's Code once live." },
  school_routes: { group: "grp_daily_express", sort: 4, icon: "directions_bus", display_template: "{{code}} — {{name}}", note: "Home-to-school route timetables." },
  // Bookings & payments
  bookings: {
    group: "grp_orders",
    sort: 1,
  ...
  service_runs: {
    ...
    icon: "event_seat",
    color: "#2563eb",
    display_template: "{{service_date}} · {{departure_time}} · {{route_slug}}",
    archive_field: "status",
  ...
  quote_requests: { group: "grp_leads", sort: 2, icon: "request_quote", display_template: "{{name}} · {{trip_from}} → {{trip_to}}", sort_field: "created_at", note: "Private-hire quote requests. Delivery fields show whether customer and staff email handling completed." },
  // Accounts
...
const FIELD_META = {
  bookings: [
    ...
    { field: "return_service_name", meta: { width: "half", readonly: true } },
  ...
  service_runs: [
    ...
    { field: "departure_time", meta: { width: "half", readonly: true } },
    { field: "capacity", meta: { width: "half", options: { min: 1, max: 500 }, note: "Dated sellable capacity (1–500). Increase this for an extra/larger bus on this date; never reduce below Booked seats." } },
    { field: "booked_seats", meta: { width: "half", readonly: true } },
    { field: "status", meta: { width: "half", interface: "select-dropdown", display: "labels", options: { choices: [{ text: "Scheduled", value: "scheduled", color: "#16a34a" }, { text: "Cancelled", value: "cancelled", color: "#dc2626" }] }, display_options: { showAsDot: true } } },
  ],
  pass_purchases: [
  ...
  routes: [
    ...
    { field: "capacity", meta: { width: "half", note: "Sellable seats per departure. Keep at 0 until operations approves online sales; 0 blocks checkout and no Service Run is created." } },
  ...
  scheduled_services: [
    { field: "code", meta: { width: "half", note: "Immutable stable code. Do not change after first use." } },
    { field: "route_slug", meta: { width: "half", note: "Parent route/SEO page slug." } },
    { field: "name", meta: { width: "half" } },
    ...
    { field: "capacity", meta: { width: "half", options: { min: 1, max: 500 }, note: "Default sellable capacity per date (1–500). Starts at 60; enter combined seats when multiple/larger buses normally operate." } },
  ...
  contact_submissions: [
    { field: "name", meta: { width: "half", readonly: true } },
    { field: "email", meta: { width: "half", readonly: true } },
    { field: "phone", meta: { width: "half", readonly: true } },
    { field: "subject", meta: { width: "half", readonly: true } },
    { field: "message", meta: { width: "full" } },
    { field: "confirmation_email_status", meta: { hidden: false, readonly: true, width: "half", display: "labels", options: { choices: EMAIL_STATUS_CHOICES }, display_options: { choices: EMAIL_STATUS_CHOICES, showAsDot: true }, note: "Customer acknowledgement; failed delivery is retried by maintenance." } },
    { field: "confirmation_email_started_at", meta: { hidden: false, readonly: true, width: "half" } },
    { field: "confirmation_email_sent_at", meta: { hidden: false, readonly: true, width: "half" } },
    { field: "staff_email_status", meta: { hidden: false, readonly: true, width: "half", display: "labels", options: { choices: EMAIL_STATUS_CHOICES }, display_options: { choices: EMAIL_STATUS_CHOICES, showAsDot: true }, note: "Independent operational copy sent to the contact inbox." } },
    { field: "staff_email_started_at", meta: { hidden: false, readonly: true, width: "half" } },
    { field: "staff_email_sent_at", meta: { hidden: false, readonly: true, width: "half" } },
    { field: "email_status", meta: { readonly: true, width: "half", display: "labels", options: { choices: EMAIL_STATUS_CHOICES }, display_options: { choices: EMAIL_STATUS_CHOICES, showAsDot: true }, note: "Overall status for both customer and staff messages." } },
    { field: "email_started_at", meta: { readonly: true, width: "half" } },
    { field: "email_sent_at", meta: { readonly: true, width: "half" } },
  ...
  quote_requests: [
    ...
    { field: "email", meta: { width: "half", readonly: true } },
    { field: "phone", meta: { width: "half", readonly: true } },
    { field: "passengers", meta: { width: "half", readonly: true } },
    { field: "trip_from", meta: { width: "half", readonly: true } },
    { field: "trip_to", meta: { width: "half", readonly: true } },
    { field: "trip_date", meta: { width: "half", readonly: true } },
    { field: "return_date", meta: { width: "half", readonly: true } },
    { field: "coach_size", meta: { width: "half", readonly: true } },
    { field: "message", meta: { width: "full", readonly: true } },
    { field: "confirmation_email_status", meta: { hidden: false, readonly: true, width: "half", display: "labels", options: { choices: EMAIL_STATUS_CHOICES }, display_options: { choices: EMAIL_STATUS_CHOICES, showAsDot: true }, note: "Customer acknowledgement; failed delivery is retried by maintenance." } },
    { field: "confirmation_email_started_at", meta: { hidden: false, readonly: true, width: "half" } },
    { field: "confirmation_email_sent_at", meta: { hidden: false, readonly: true, width: "half" } },
    { field: "staff_email_status", meta: { hidden: false, readonly: true, width: "half", display: "labels", options: { choices: EMAIL_STATUS_CHOICES }, display_options: { choices: EMAIL_STATUS_CHOICES, showAsDot: true }, note: "Independent operational copy sent to the quote inbox." } },
    { field: "staff_email_started_at", meta: { hidden: false, readonly: true, width: "half" } },
    { field: "staff_email_sent_at", meta: { hidden: false, readonly: true, width: "half" } },
    { field: "email_status", meta: { readonly: true, width: "half", display: "labels", options: { choices: EMAIL_STATUS_CHOICES }, display_options: { choices: EMAIL_STATUS_CHOICES, showAsDot: true }, note: "Overall status for both customer and staff messages." } },
    { field: "email_started_at", meta: { readonly: true, width: "half" } },
  ...
  settings: [
    ...
    { field: "school_transport", meta: { width: "full", interface: "input-code", options: { language: "json" }, note: "Home-to-school ticket buttons and availability. Edit ticketPortalUrl and each school's buyUrl, waitlistUrl and spacesAvailable values. Logos are seeded once, then CMS-managed." } },
    { field: "faqs", meta: { width: "full", note: "Homepage and FAQ-page questions and answers." } },
    { field: "hero_image", meta: { width: "half", note: "Optional homepage video poster/fallback image. Clear it for video only; deployments preserve your choice." } },
    { field: "hero_image_alt", meta: { width: "half", note: "Accessible description of the homepage hero image." } },
    { field: "hero_video", meta: { width: "full", note: "Homepage video takes priority over the image. Use a browser-playable MP4 (H.264). Muted autoplay respects reduced-motion/data-saving preferences. Remove it for image only." } },
    ...
    { field: "page_hero_fallback_image_1", meta: { width: "half", note: "Fallback hero 1. Used automatically only when a page has no assigned image." } },
    { field: "page_hero_fallback_image_1_alt", meta: { width: "half", note: "Accessible description for fallback hero 1." } },
    { field: "page_hero_fallback_image_2", meta: { width: "half", note: "Fallback hero 2. Pages are distributed consistently across the available fallback images." } },
    { field: "page_hero_fallback_image_2_alt", meta: { width: "half", note: "Accessible description for fallback hero 2." } },
    { field: "page_hero_fallback_image_3", meta: { width: "half", note: "Fallback hero 3. Clear a slot to remove it from the fallback pool." } },
    { field: "page_hero_fallback_image_3_alt", meta: { width: "half", note: "Accessible description for fallback hero 3." } },
